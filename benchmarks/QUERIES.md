...
All queries run against both systems using `benchmarks/query_benchmark.sh`.
Same time range applied to both: **June 2–3, 2026 full 2 days**
...
| # | Query Name | Type | Elasticsearch Query | O2 SQL |
|---|---|---|---|---|
  ...
  "query": {
    "bool": {
      ...
      "filter": {
        "range": {
          "_timestamp": {
            "gte": 1780358400000000,
            "lte": 1780531199000000
          }
        }
      }
    }
...
POST /api/default/_search?use_cache=true
{
  "query": {
    "sql": "<SQL query>",
    "start_time": 1780358400000000,
    "end_time": 1780531199000000,
    "from": 0,
    "size": 100
  }
}
...
- **O2 caching**: `use_cache=true` enabled — recommended production configuration
